#!/usr/bin/env python3
"""Reproducible logistic-regression classification audit for Exercise 4 (WP17).

Exercise 4 predicts autism diagnosis from cortical structure -- the first
classification exercise in the course. Unlike the WP12/WP13 regression and
KNN audits, this script does NOT search over feature spaces, hyperparameters,
or seeds: WP17 requires one project-standard deterministic seed and one fixed,
ordinary regularization setting, chosen before inspecting test performance,
so there is nothing to select here. This script exists to compute that one
honest result once, reproducibly, and commit it for the notebook and its
tests to both cite -- not to search for a better one.
...
* model: ``Pipeline(StandardScaler(), LogisticRegression(C=1.0,
  max_iter=5000))``. The scaler is fit on training rows only. C=1.0 is fixed,
...
def _pipeline():
    ...
    return make_pipeline(StandardScaler(), LogisticRegression(C=C_FIXED, max_iter=MAX_ITER))
...
def _fit_eval(X_train, y_train, X_test, y_test) -> dict[str, Any]:
    import numpy as np
...
def run_audit() -> dict[str, Any]:
    ...
    first = _fit_eval(X_train, y_train, X_test, y_test)
    second = _fit_eval(X_train, y_train, X_test, y_test)
    if first != second:
    ...
    results: dict[str, Any] = {
        ...
        "protocol": {
            ...
            "model": f"Pipeline(StandardScaler(), LogisticRegression(C={C_FIXED}, max_iter={MAX_ITER}))",
            "note": (
                "C is a fixed, ordinary regularization setting chosen before inspecting test "
                "performance; it is never tuned on the test set. No feature, seed, or model "
                "selection was performed to improve this result."
            ),
        },
        "cohort": {
            "n_total": int(len(y)),
