...
for (const { name, prefix } of BASES) {
  test.describe(`table-inspection @ ${name}`, () => {
    test("head/tail are deterministic pandas-equivalents; sample is a fixed seeded draw", async ({
      ...
      // default view = head(8): the first eight rows, all from one site, no
      // missing cells in the displayed columns (independently cross-checked with
      // pandas over the real 1114-row table).
      ...
      await expect(page.locator('[data-testid="table-inspection-summary"]')).toContainText(
        "head() shows 8 rows from 1 acquisition site",
      );

      // switch to tail(8): a different single site, and missing cells appear
      ...
      await expect(table).toHaveAttribute("data-missing-cells", "19");
    ...
    test("row-count control changes rows and the evidence summary", async ({ page }) => {
      ...
      await expect(table).toHaveAttribute("data-total-cells", "120"); // 15 rows * 8 cols
    ...
    test("missing cells are marked accessibly and are not called errors", async ({ page }) => {
      ...
      expect(await missing.count()).toBe(19);
    ...
    test("is usable at a narrow (mobile) viewport and the table scrolls within its box", async ({
      page,
    }) => {
      ...
      expect(scrollBox?.width ?? 999).toBeLessThanOrEqual(375);
    });
  });
}
