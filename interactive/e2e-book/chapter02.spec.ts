...
// Exercise 2 HTML page, served beneath the simulated GitHub Pages project
// subpath -- not just the standalone widget page. The regression-compare
// activity previously covered here moved to Exercise 5 in WP28 (see
// chapter05.spec.ts); Exercise 2's built page now embeds knn-explore only.

const CHAPTER_URL = "/ml-neuro-tutorials/chapters/chapter_02/exercise_02.html";

const KNN_IFRAME_SELECTOR =
  'iframe[title="Interactive KNN neighbour-count exploration for predicting age from brain structure"]';
const KNN_N_FIT = 564;

async function knnFrame(page: import("@playwright/test").Page): Promise<Frame> {
  const handle = await page.locator(KNN_IFRAME_SELECTOR).elementHandle();
...
test.describe("Chapter 2 built page — structure", () => {
  test("embeds exactly one activity (knn-explore); regression-compare moved to Exercise 5", async ({
    page,
  }) => {
    await page.goto(CHAPTER_URL);
    await expect(page.locator("iframe")).toHaveCount(1);
    await expect(page.locator(KNN_IFRAME_SELECTOR)).toHaveCount(1);
    await expect(
      page.locator(
...
test.describe("Chapter 2 built page — embedded KNN k-exploration", () => {
  test("iframe loads, config+data are 200, both plots render, a real k change is real", async ({
    ...
    await page.goto(CHAPTER_URL);
    const iframe = page.locator(KNN_IFRAME_SELECTOR);
    await expect(iframe).toHaveCount(1);
    await iframe.scrollIntoViewIfNeeded();

    const frame = await knnFrame(page);
  ...
  test("k = N_fit collapses the scatter plot to the fitting-set mean", async ({ page }) => {
    await page.goto(CHAPTER_URL);
    await page.locator(KNN_IFRAME_SELECTOR).scrollIntoViewIfNeeded();
    const frame = await knnFrame(page);
    await expect(frame.locator("#app")).toHaveAttribute("data-widget-ready", "true");

    await frame.locator('[data-testid="knn-k-slider"]').fill(String(KNN_N_FIT));
    await expect(frame.locator('[data-testid="knn-k-value"]')).toHaveText(String(KNN_N_FIT));
    const metrics = await frame.locator('[data-testid="knn-metrics"]').innerText();
